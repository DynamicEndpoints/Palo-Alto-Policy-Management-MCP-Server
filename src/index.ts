...
class PolicyServer {
  ...
  private setupToolHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request: CallToolRequest) => {
      try {
        const args = request.params.arguments as RuleData;
        ...
        switch (request.params.name) {
          ...
          case 'create_rule':
            if (!args.rule_type || !args.rule_data) {
              throw new McpError(
                ...
                'Missing required parameters: rule_type and rule_data'
              );
            }
          ...
          case 'update_rule':
            if (!args.rule_type || !args.rule_name || !args.rule_data) {
              throw new McpError(
                ErrorCode.InvalidParams,
                'Missing required parameters: rule_type, rule_name, and rule_data'
              );
            }
          ...
          case 'delete_rule':
            if (!args.rule_type || !args.rule_name) {
              throw new McpError(
                ErrorCode.InvalidParams,
                'Missing required parameters: rule_type and rule_name'
              );
            }
        ...
        if (error instanceof AxiosError) {
          throw new McpError(
            ...
            `Palo Alto API error: ${error.message}`
          );
        }
        throw new McpError(
          ErrorCode.InternalError,
  ...
  private async getRules(ruleType: string) {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/Policies/${ruleType}Rules`,
        {
          headers: {
            'X-PAN-KEY': API_KEY,
  ...
  private async createRule(ruleType: string, ruleData: object) {
    try {
  ...
  private async updateRule(ruleType: string, ruleName: string, ruleData: object) {
    try {
      const response = await axios.put(
        `${API_BASE_URL}/Policies/${ruleType}Rules/${ruleName}`,
        ruleData,
        {
          headers: {
            'X-PAN-KEY': API_KEY,
  ...
  private async deleteRule(ruleType: string, ruleName: string) {
    try {
